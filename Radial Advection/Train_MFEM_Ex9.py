...
#         plt.show()
    ...
    ex = np.load('./data/ex{}_5.npz'.format(example), allow_pickle = True)
    data = ex.f.arr_0
    if example % 9 == 0:
...
# #     fig = plt.figure()
# #     ax_init = plt.axes()
# #     ax_init.pcolormesh(X,Y, ex[0].reshape(512,512), shading = 'nearest')
# #     ax_init.axis('equal')


# #     fig = plt.figure()
# #     ax_final = plt.axes()
# #     ax_final.pcolormesh(X,Y, ex[-1].reshape(512,512), shading = 'nearest')
# #     ax_final.axis('equal')
...
#     data = ex.reshape(-1,512,512)[:,::8,::8]
#     data = data.reshape(-1,nx*ny)
    
#     fig = plt.figure()
#     ax_init = plt.axes()
#     ax_init.pcolormesh(X[::4,::4],Y[::4,::4], data[-1].reshape(nx,ny), shading = 'nearest')
#     ax_init.axis('equal')
...
#     plt.title('Original $u$')
    ...
    PATH = './checkpoint_ex{}_64.tar'.format(example)
    ...
    # save models
    print()
    print("Saving after {}th training to".format(epoch),file_name_AE)
    encoder = encoder
    decoder = decoder
    torch.save({'encoder_state_dict': encoder.state_dict(), 'decoder_state_dict': decoder.state_dict()}, '../../../../../usr/WS1/fries4/Experiments/EX9/model/ex{}_5_AE_{}.tar'.format(example, LS_dim))
...
# fig = plt.figure()
# ax_u = plt.axes()
# ax_u.set_title('Latent Space')
# latent_space_SS = []

# colors = ['b','m','y','g','c','r']
# styles = ['-', '--', ':', '-.', (0, (3, 5, 1, 5, 1, 5))]
# for foo in range(nset):
#     input_SS=torch.tensor(orig_data[foo*(nt+1):(foo+1)*(nt+1)].astype('float32')).to(device)
...
# AE_recon = decoder(encoder(input_SS)).cpu().detach().numpy()

# k=0

# fig = plt.figure()
# ax = plt.axes()
# z = ax.pcolormesh(x_free.reshape(ny-2,nx-2), y_free.reshape(ny-2,nx-2), AE_recon[k].reshape(ny-2,nx-2)-orig_data[k].reshape(ny-2,nx-2), shading = 'nearest',cmap=cm.viridis)
# fig.colorbar(z)

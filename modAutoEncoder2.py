...
def getDataLoaders(training_data, test_data, batch_size):
  ...
  try:
    ...
    train_sampler = DistributedSampler(dataset=dataset['train'], shuffle=True) 
    test_sampler = DistributedSampler(dataset=dataset['test'], shuffle=True)
    
    train_loader = DataLoader(dataset=dataset['train'], batch_size=batch_size,
                              sampler=train_sampler, num_workers=4, pin_memory=True)
    test_loader = DataLoader(dataset=dataset['test'], batch_size=batch_size,
                              shuffle=True, sampler=test_sampler, num_workers=4)
  except:
...
def trainAE( encoder, 
  ...
  # set data loaders
  train_loader, test_loader = getDataLoaders(training_data, test_data, batch_size)  
  ...
  for epoch in range(last_epoch+1,num_epochs+1):   
          ...
          for data, in data_loaders[phase]:
              ...
              if phase == 'train':
                  if scheduler != None:
                      ...
                      # forward
                      outputs = decoder(encoder(inputs.float()))
      ...
      if (epoch%num_epochs_print==0) and (is_main_process()):
          torch.save({
                      'epoch': epoch,
      ...
      if (epoch%num_epochs_save_model==0) and (is_main_process()):
...
# following functions from: https://theaisummer.com/distributed-training-pytorch/
